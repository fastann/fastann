...
impl KmeansIndexer{
    ...
    pub fn get_distance_from_vec(&self, x: &Vec<f64>, y: &Vec<f64>) -> f64{
        return calc::enclidean_distance_range(x,y, self._data_range_begin,self._data_range_end).unwrap();
    }
    ...
    fn update_center(&mut self, batch_size: usize, batch_data: &Vec<Vec<f64>>, assigned_center: &Vec<usize>) -> Result<Vec<usize>, &'static str>{
        let demension = self._demension;
        let n_center = self._n_center;
        let begin = self._data_range_begin;
        let mut new_centers: Vec<Vec<f64>> = Vec::new();
    ...
    fn search_data(&mut self, batch_size: usize, batch_data: &Vec<Vec<f64>>, assigned_center: &mut Vec<usize>){
        let n_center = self._n_center;
        let demension = self._demension;
        for i in 0..batch_size{
    ...
    fn split_center(&mut self, batch_size: usize, n_assigned_per_center: &mut Vec<usize>) -> Result<(), &'static str>{
        let demension = self._demension;
        let n_center = self._n_center;
        ...
        for i in 0..n_center{
            if n_assigned_per_center[i] == 0{//rand pick split center
                let mut split_center_id = i + 1;
                loop {
                    let mut rng = rand::thread_rng();
                    let pick_percent = n_assigned_per_center[split_center_id] as f64 / batch_size as f64;
                ...
                for j in 0..demension{
                    if j%2 == 0{
                        ...
                        self._centers[split_center_id][j] = self._centers[split_center_id][j] * (1.0 + EPS);
                    }
                    else{
                        self._centers[i][j] = self._centers[split_center_id][j] * (1.0 + EPS);
                        self._centers[split_center_id][j] = self._centers[split_center_id][j] * (1.0 - EPS);
    ...
    pub fn train(&mut self, batch_size:usize , batch_data: &Vec<Vec<f64>>){
        ...
        for epoch in 0..self._n_epoch{
            ...
            let mut n_assigned_per_center = self.update_center(batch_size, batch_data, &assigned_center).unwrap();
...
impl PQIndexer{
    ...
    pub fn get_distance_from_vec_range(&self, x: &Vec<f64>, y: &Vec<f64>, begin:usize, end:usize) -> f64{
        return calc::enclidean_distance_range(x,y, begin, end).unwrap();
    }

    pub fn search_knn_ADC(&self, search_data: &Vec<f64>, k: usize) -> Result< BinaryHeap<Neighbor>, &'static str>{
        ...
        for i in 0..self._n_sub{
            ...
            for j in 0..self._n_sub_center{
                sub_dis.push(self.get_distance_from_vec_range(search_data, &self._centers[i][j], i*self._n_sub, (i+1)*self._n_sub));
            }
